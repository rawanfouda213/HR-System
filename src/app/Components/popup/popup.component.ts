...
export class PopupComponent implements OnInit{
  ...
  setpopupdata(code: any) {
    this.service.GetCustomerbycode(code).subscribe(item => {
      this.editdata = item;
      this.EmployeeSettingsForm.setValue({name:this.editdata.name,email:this.editdata.email,phone:this.editdata.phone,
  ...
  EmployeeSettingsForm = this.buildr.group({
  ...
  Saveuser() {
    this.service.Savecustomer(this.EmployeeSettingsForm.value).subscribe(res => {
      this.closepopup();
    });
  }
}